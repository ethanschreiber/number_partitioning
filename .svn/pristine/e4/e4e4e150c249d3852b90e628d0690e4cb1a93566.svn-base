/* This program optimally solves arbitrary K-way partitioning problems
   using the extended Schroeppel and Shamir algorithm and iterative
   partitioning.  First, the KK heuristic is used to compute an
   approximate K-way partition.  Then, it generates a first subset in
   the appropriate subset sum range, including the largest number.  It
   then recursively calls the program to partition the remaining
   numbers K-1 ways.  It terminates partitioning the remaining numbers
   when it finds a partition of them whose largest subset sum is less
   than or equal to the largest subset sum of the completed subsets in
   the current partition being explored.  It also prunes completions
   where the smallest excluded number could be added to the current
   subset sum without exceeding the largest subset sum in the
   completed subsets of the current partition. This version uses a
   generic version of CGA, rather than special-case versions for 3, 4,
   and 5-way partitioning. This version prunes nodes if any excluded
   element can be swapped with all the smaller included elements
   without exceeding MAXSOFAR. This version doesn't maintain a global
   variable for the best solution found so far, but passes it as a
   parameter to each recursive call. This version also prunes nodes if
   any excluded element can be swapped with the next smaller included
   element without exceeding MAXSOFAR. This version maintains all the
   subsets sorted in decreasing order. */
...
int64_t diffthresh;       /* difference threshold at which to terminate CKK */

int64_t calls[MAXK+1];   /* counters for number of partitionings for each K */
...
int64_t A[MAXN]; /* global array of numbers to be partitioned by CKK or CGA */
int64_t sums[MAXK];                                 /* array of subset sums */
int cgan;                              /* number of numbers to be partitioned */
int cgak;                         /* number of subsets to partition them into */
int64_t beta;                                 /* best solution found by CGA */
...
{int i;                                                   /* index into array */
 ...
 void sortsup (int64_t a[MAXN], int n)
...
int64_t kk2 (
     int64_t a[MAXN],                       /* array of subset sum vectors */
     int n)                                    /* number of elements in array */

{int64_t b[MAXN];                      /* copy of original array to destroy */
  int i;                                                  /* index to vectors */
  int j;                         /* index to individual subset sums in vector */
  int64_t diffOneTwo;  /* new number which is difference between first two numbers */
...
void gensums (int64_t nums[MAXN],                          /* array of original numbers */
...
void ckk (int64_t a[MAXN],                                        /* array of numbers */
int n,                                         /* number of elements in array */
...
int64_t twopart
(int64_t nums[MAXN],                 /* array of integers to be partitioned */
 int n,                               /* number of integers to be partitioned */
 int64_t sum,                                    /* sum of all the integers */
 int64_t maxsofar, /* largest subset sum so far in the current solution path*/
 int64_t bestsofar) /* largest subset sum in best complete solution found so far */

{int i;                                                      /* utility index */
 int64_t best;                           /* best partition difference found */
 int64_t target1;       /* two target values, equal if sum is even */
...
{int64_t largestOfLastTwo;                      /* largest subset sum of last two subsets */
...
int64_t
part (int64_t num[MAXN],              /* array of integers to be partitioned */
      int n,                                    /* number of integers in array */
      int64_t sum,                                /* sum of all the integers */
      int k,                       /* number of ways to partition the integers */
      int64_t maxsofar, /*largest sum among completed subsets in current path*/
      int64_t bestsofar) /* largest subset sum in best complete solution found so far */
...
{int64_t lower;                       /* lower bound of top-level subset sum */
  ...
  int64_t x[MAXN]; /* array to contain integers excluded from current subset */
  ...
  int64_t sol;             /* maximum subset sum from recursive subpartition */
  ...
  int nexty;           /* temporary for y component of next element of CD heap */
  int64_t largest;                         /* largest number in original set */
  int64_t newmax; /* new maximum subset sum of completed subsets on current path*/
  int64_t sumrest;       /* sum of remaining included items in current subst */
  int64_t rest;                                  /* sum of excluded elements */
  int64_t lastex;       /* smallest excluded number so far in current subset */
...
uint64_t executeSNP(const partition::PartitionProblem &problem, ProblemStats &stats) {
	...
	int64_t perfect = 0;               // largest subset sum in perfect partition
	int64_t minmax;          // the largest subset sum in an optimal solution

	int64_t S[MAXN]; // array of integers for each problem instance
	...
	int64_t sumall = problem.sum; // sum of all the numbers in the problem instance
	...
	if (K == 2)
		minmax = twopart(S, N, sumall, sumall - sumall / 2, sumall); // optimal 2-way
	else
		minmax = part(S, N, sumall, K, perfect, sumall); // optimally partition K ways
