...
#        t1 = lib.processExperiment(lib.filename(K,N,alg2,numExperiments),minCount)
        t0 = lib.processExperiment(lib.filename(K,N,alg1,numExperiments),count0)
        t1 = lib.processExperiment(lib.filename(K,N,alg2,numExperiments),count1)
            
        if ((count0 == 0) or (count1 == 0) or (t0 == 0) or (t1 == 0)) :
            ratio = "-"
        else :
            if (t1 > t0) :
                value = int(round(t1 / t0))
                ratio = str(value)
            else :
                value = int(round(t0 / t1))

                if value > 1000 :
                    ratio = "1/" + str(value/1000) + "k"
                elif value > 1 :                    
                    ratio = "1/" + str(value)
                else :
                    ratio = str(value)
