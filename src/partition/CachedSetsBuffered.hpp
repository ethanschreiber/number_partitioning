...
class CachedSetsBuffered {
	protected :
		...
		SetVector m_smallSets;      		// The sets with sum < perfect generated by Schroeppel and Shamir
		SetVector m_largeSets;      		// The sets with sum > perfect generated by Schroeppel and Shamir
		size_t m_smallSetIdx;       		// The idx into m_smallSets
		size_t m_largeSetIdx;       		// The idx into m_largeSets

		SetVectorIt m_currentMax; 			// The next max value to try, taken from m_maxHeap
		SetVectorIt m_currentMinBegin;	// The beginning of the next min values to try, taken from m_minHeap
		SetVectorIt m_currentMinEnd;		// The beginning of the next min values to try, taken from m_minHeap
...
class CachedSetsBufferedSS : public CachedSetsBuffered {
	...
	public :
		CachedSetsBufferedSS(const uint64_t S[], const int N, const int K,
		    uint64_t upperBound, size_t numSets);
};
...
private :
  ...
  //ss::SchroeppelShamirMSubsets m_ssms;
  ss::HorowitzSahniLowCardinalityMSubsets m_hsms;
